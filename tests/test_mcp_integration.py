...
from skislope_mcp.elevation.server import elevation_provider, server
...
class TestMCPIntegration:
    ...
    def test_data_source_configuration(self):
        ...
        for _source_name, source_config in elevation_provider.data_sources.items():
    ...
    async def test_end_to_end_elevation_fetch(self, mock_urlopen):
        ...
        assert len(result) == 4
        assert all(isinstance(elev, numbers.Real) for elev in result)
